...
### P3 — 제품 확장 (이전 논의)
- [x] ~~공유 즐겨찾기 리스트 → 지도 뷰~~ (8/16 v1 완료 — `shared_lists` 스냅샷 테이블+RLS(`0006_shared_lists.sql`, 오너가 직접 적용), Saved 탭 "Share this list"(게스트=join sheet), `/map?list={uuid}` 핀 레이어+배너+Save all, 깨진 링크는 토스트+일반 지도 폴백. **프로덕션 왕복 E2E 통과** — 회원 공유→게스트 열람→Save all→가입 유도. OG 미리보기 카드 8/16 완료(generateMetadata, 프로덕션 E2E 검증). 백로그 잔여: 리스트 이름 직접 입력)
