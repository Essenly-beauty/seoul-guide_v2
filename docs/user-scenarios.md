...
## 갭 분석 — 페르소나별 부족한 부분 (2026-07-26)

### P1 Mia (관광객·쇼핑) 갭
| 갭 | 내용 | 우선순위 |
|---|---|---|
| 세금환급 실행 안내 | 국적을 물을 때 약속한 가치("tax-refund tips")가 화면에 없음 — Tax-free 뱃지만 존재. 환급 최소금액·방법·공항 수령 안내 필요 | 높음 |
| 여행 일정 활용 | stay_until을 받지만 쓰지 않음 — "출국 D-2: 환급 서류 챙기기" 류 체크리스트/리마인드 부재 | 중간 |
| 결제·환율 힌트 | 외국 카드 결제 가능 여부는 있으나 환율/팁 관련 안내 없음 | 낮음 |

### P2 Yuki (시술) 갭
| 갭 | 내용 | 우선순위 |
|---|---|---|
| 시술 가격 비교 | 같은 시술(예: 아쿠아필)을 여러 클리닉끼리 비교하는 뷰 없음 — 상세 개별 확인만 가능 | 높음 |
| 예약 커뮤니케이션 | 채널로 넘긴 뒤 언어 장벽 — 예약 요청 한글 템플릿(복사용) 제공하면 해소 | 중간 |
| 시술 후 관리 | 사후관리 안내·후속 방문 리마인드 부재 | 낮음 |

### P3 Sarah (거주) 갭
| 갭 | 내용 | 우선순위 |
|---|---|---|
| 방문 기록 기반 재방문 | "지난달 간 살롱, 다시 예약?" 류 단골 루프 없음 — 별점만 저장 | 중간 |
| 서버 동기화 | 찜/프로필이 기기 로컬 — 로그인 후 기기 간 동기화는 DB 단계 과제 | (로드맵 반영됨) |

### 공통 갭
- **다국어**: UI 영어 고정 — 일본어/중국어 수요(P2) 미커버
- **콘텐츠 연결(P4)**: Stories→장소/상품 링크가 단방향 — 아래 P4로 해소

### P4 (신규) — 외부 콘텐츠 유입 사용자
> "블로그에서 이 에센스 봤는데 어디서 사?" / "건성 진단 받았는데 뭘 사고 어느 샵을 가?"

| 니즈 | 대응 (이번 구현) |
|---|---|
| 제품 → 오프라인 구매처 | 상품 상세 **Where to buy**: 판매 채널·존 기반 실제 지점(올리브영/백화점) 가까운 순 + LIVE |
| 고민 → 케어샵 | 상품 상세 **Care shops · {concern}**: 고민↔시술 태그 매핑으로 관련 클리닉/샵 추천 |
| 고민 → 제품 탐색 | 랭킹 **Shop by concern** 칩 필터 + `/ranking?concern=` 딥링크(블로그에서 바로 링크 가능) |
| 고민 검색 | "hydration", "dry skin" 검색 시 제품 매칭 |
| 진입점 | 검색 빈 화면에 Shop by concern 바로가기 |
