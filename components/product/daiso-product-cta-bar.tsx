...
import { daisoBrandEn, daisoNameEn } from "@/lib/daiso-ranking-en";

export function DaisoProductCtaBar({ product }: { product: DaisoRankingProduct }) {
  const shareProduct = {
    id: product.id,
    brand: daisoBrandEn(product.productNo) || product.brand,
    name: daisoNameEn(product.productNo) || product.nameKr,
  };
